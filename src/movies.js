...
const movieyear = movies.sort((a,b) => a.year === b.year ? (b.title < a.title ? 1 : -1) : a.year - b.year).slice(0,20);
//
...
const stevenMovies = movies.filter(obj => obj.director === 'Steven Spielberg' && obj.genre.includes("Drama")).fill(1).reduce((acc,x)=> acc + x);
// console.log(stevenMovies)

// function howManyMovies (arr){
//  var sbMovies = 0
//  var filtered = arr.filter(obj =>{
...
let lexOrder = movies.sort((a,b)=> a.title > b.title ? 1 : -1).slice(0,20);
...
//         return -1;}});
//     let sampleBatch = sorted.slice(0,20);
//     let titles = sampleBatch.map(movie => {
//         return movie.title;});
//     return titles;}
...
const averageRates = (movies.reduce((acc,a) => acc + a.rate, 0) / movies.length).toFixed(2)
...
const dramaMoviesRate = (movies.filter(a => a.genre.indexOf("Drama")).reduce((acc,a)=> acc + a.rate,0) / movies.length);
// console.log(dramaMoviesRate);


// function dramaMoviesRate(arr) {
  // return ratesAverage(arr.filter(a => curr.genre.indexOf("Drama") >= 0)) || 0;}
...
const turnHoursToMinutes = movies.map(x =>  x.duration.toString().replace(/1h/gi,60).replace(/2h/gi,120).replace(/3h/gi,180).replace(/min/gi,'').split(" "));
...
//     }
//   }
//   return {...movie,duration: mins + ' min'};
// });
// }
